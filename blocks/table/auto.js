...
$.fn.createTableEditor = function (options) {
  ...
  var $container = this;
  var controls = $("\n        <div class=\"input-group mb-3\">\n            <select class=\"form-select columnSelect\">\n                <option value=\"\">".concat(settings.selectColumnText, "</option>\n            </select>\n            <button class=\"btn btn-danger remove-column-btn\">").concat(settings.removeColumnText, "</button>\n            <button class=\"btn btn-primary add-column-btn\">").concat(settings.addColumnText, "</button>\n            <button class=\"btn btn-success add-row-btn\">").concat(settings.addRowText, "</button>\n        </div>\n        <div class=\"mt-3 table-container\"></div>\n    "));
  $container.append(controls);
  var tableColumns = [],
    data = [];
  if (typeof settings.data !== "undefined" && settings.data !== null && settings.data.length > 0) {
    data = settings.data;
    tableColumns = Object.keys(settings.data[0]).map(function (field) {
      return {
        title: field,
        field: field,
        editor: "input"
  ...
  var table = new tabulator_tables__WEBPACK_IMPORTED_MODULE_0__.TabulatorFull($container.find(".table-container").get(0), {
  ...
  function saveCode() {
    var visibleColumns = table.getColumns().map(function (col) {
      return col.getField();
    }).filter(function (field) {
      return field;
    });
    var jsonData = table.getData().map(function (row) {
      var filteredRow = {};
  ...
  function updateColumnDropdown() {
    var columnSelect = $container.find('.columnSelect');
    columnSelect.empty();
    columnSelect.append("<option value=\"\">".concat(settings.selectColumnText, "</option>"));
    table.getColumns().forEach(function (col) {
      var field = col.getField();
      if (field) {
        columnSelect.append("<option value=\"".concat(field, "\">").concat(col.getDefinition().title, "</option>"));
      }
    });
  }
  $container.find('.add-column-btn').on("click", function (event) {
    event.preventDefault();
    var fieldName = prompt(settings.addColumnText);
    if (!fieldName) return;
    table.addColumn({
      title: fieldName,
      field: fieldName,
  ...
  $container.find('.remove-column-btn').on("click", function (event) {
    event.preventDefault();
    var selectedColumn = $container.find('.columnSelect').val();
    if (!selectedColumn) {
...
/******/ 		}
/******/ 		
/******/ 		var chunkLoadingGlobal = self["webpackChunksimple_tables"] = self["webpackChunksimple_tables"] || [];
...
/******/ 	__webpack_require__.O(undefined, ["table/auto"], () => (__webpack_require__("./assets/table.js")))
/******/ 	var __webpack_exports__ = __webpack_require__.O(undefined, ["table/auto"], () => (__webpack_require__("./assets/table.scss")))
